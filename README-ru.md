...
**Примечание:** В файле `env` НЕ заключайте значения в `""` или `''` и не добавляйте пробелы вокруг `=`. Если вы изменили `LITELLM_PORT`, обновите флаг `-p` в команде `docker run` соответствующим образом.
